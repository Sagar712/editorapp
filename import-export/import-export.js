...
inputDial.addEventListener('change', function(e){
    ...
    reader.onload = function(){
        try{
            let abc = inputDial.files[0].name.toString();
            if(abc.includes('.html')){
                const allData = reader.result;
                let splits = allData.split('<!-------------------------------------------------------->');
                console.log(splits[1]);
                localStorage.setItem('textData', splits[1]);
                alert("import successful !");
            }
            else{
                alert("import Failed !\nPossible reasons:"+
            "\n• wrong name\n• extension is not .html\n• file corrupted(very rare)");
            }
...
function exportFile() {
    ...
    fullString = fullString.concat(`<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        ...
        <title>Text editor Backup</title>
    </head>
    <body>
